                ...
                <div class="trip-info-box">
                    ...
                    <form action="<?php echo base_url(); ?>Book/Reserved" method="POST"  role="form"  id="myTable">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="date" id="datepicker" class="form-control" value="" required="required" titl>    
                        </div>
                         <?php if ($product['product_name'] == 0) { ?>
                        <div class="form-group">
...
                            //$('#id').val(data.id);
                            ...
                            //reset total_price
                            //alert(data.date);
                            var disableddates = [data.date_stop];
                            function DisableSpecificDates(date) {
                            var string = jQuery.datepicker.formatDate('dd-mm-yy', date);
                                return [disableddates.indexOf(string) == -1];
                            }
                            
                            var sunday      = parseInt(data.sunday);
                            var munday      = parseInt(data.munday);
                            var tuesday     = parseInt(data.tuesday);
                            var wednesday   = parseInt(data.wednesday);
                            var thursday    = parseInt(data.thursday);
                            var friday      = parseInt(data.friday);
                            var saturday    = parseInt(data.saturday);
                            function Disable(date){
                                                var day = date.getDay();
                                                //var month = date.getMonth();
                                                //var currDate = data.getDate();
                                                //ยังกำหนดวันที่ไม่ได้
                                                    if (day == sunday || day == munday || day == tuesday || day == wednesday || day == thursday || day == friday || day == saturday) {
                                                        return [true];
                                                    }else{
                                                        return [false];
                                                    }
                             ...
                             */
                                ...
                                $("#datepicker" ).datepicker({                                                       
                                                dateFormat: "dd/mm/yy",
                                                minDate: 2,
                                                maxDate: new Date(disableddates),                                                
